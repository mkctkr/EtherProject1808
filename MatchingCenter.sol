...
contract MatchingCenter{ 
    ...
    function registerDonator(string _name, string _bloodtype, address _patientAddr) public{
    ...
    function registerPatient(string _name, string _bloodtype) public{
    ...
    function Announce_DeterMatching() public onlyGovernor {
        ...
        for(uint i=0; i<donatorList.length; i++)
        {
            ...
            {
                if(donators[donatorList[i]].patientAddress == patientList[j] && donableChk(patientList[i], donatorList[j]) != 0)
                {
    ...
    */
    ...
    function regMatchset(address pat, address don) public onlyGovernor {
    ...
    function donableChk(address pat, address don) public onlyGovernor returns (uint){
        string patBloodTp = patients[pat].BloodType;
        string donBloodTp = donators[don].BloodType;
        
        // Full Searching matchedList
        for(uint i=0; i<matchedList.length; i++)
        {
            if( pat == matchedList[i].patientAddress || don == matchedList[i].donatorAddress )
            {
                return 0;
            }
        }
        
        return ( Strcmp(patBloodTp, donBloodTp) ) ? 1 : ( !Strcmp(patBloodTp, donBloodTp) && Strcmp(donBloodTp, "O") ? 2 : 0 );
